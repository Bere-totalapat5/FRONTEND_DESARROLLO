...
@extends('layouts.index_oralidad')
            ...
            scheduler.parse([
                @isset($lista["response"])
                    @foreach ($lista["response"] as $agenda)
                        { start_date: "{{$agenda['fecha']}} {{$agenda['hora_inicio']}}", end_date: "{{$agenda['fecha']}} {{$agenda['hora_final']}}", text:"{{$agenda['nombre']}}", section_id:"{{$agenda['ponencia']}}", id:"{{$agenda['id_evento']}}"},
                    @endforeach
                @endisset
            ]);
