  ...
  <div class="section-wrapper" style="max-width: 100%;">
          <div class="form-layout">
            ...
            <div class="d-flex justify-content-between">
                <a style="border:1px solid #ccc; width: 70px;" data-toggle="collapse" data-parent="#accordion" href="#collapseSearchAdvance" aria-expanded="false" aria-controls="collapseSearchAdvance" class="btn btn-default" >
                    <i class="fa fa-search" aria-hidden="true"></i>
                    <i class="fas fa-chevron-down" style="margin-left: 5%; font-size:0.7em;"></i> 
                </a>
                <a  style="background: #848F33 !important; color:#fff; width:80%;" data-toggle="collapse" data-parent="#accordion" href="#collapseSearchAdvance1" aria-expanded="false" aria-controls="collapseSearchAdvance1" class="btn btn-default">
...
<script>
    ...
    */
        ...
        if(version_CSV == '' || nombre_CSV == '' ||  version_CSV == null || nombre_CSV == null ){
            ...
            $.ajax({
                ...
                success:function(response) {
                    ...
                    if(response.status == 100){
                        $('#table_vista_Excel_Modal').DataTable().destroy();
                        mostrar_version_nueva_csv(id_reporte,version_CSV,json_csv, csv2);
    ...
    {{--  Menu contextual  --}}
    function ver_infoAdip(id, i_rango_fecha, version,registros, creacion, modificacion, ultimo_usuario ){

        $('#title_reporte').html('Informacion del documento '+id);
        $('#i_id_reporte').html(id);
        $('#i_rango_fecha').html(i_rango_fecha);
        $('#i_version').html(version+'<i onclick="ver_versiones(1, '+version+')" class="far fa-arrow-alt-circle-right" style="font-size:1.3em; position:absolute; right:6%; cursor:pointer;"></i>');
        $('#i_registros').html(registros);
        $('#i_creacion').html(creacion);
        $('#i_modificacion').html(modificacion);
        $('#i_ultimo_user').html(ultimo_usuario.toUpperCase());
    ...
    function ver_versiones(accion, version){
        if(accion == 1){
            $('#infoReporte_v').css('display', 'none');
            $('#modal_versiones').css({display:'inline-block', width:'100%'});
            var ver = '<div style="position:relative; height:23px;"><i onclick="ver_versiones(2,0)" class="far fa-arrow-alt-circle-left" style="font-size:1.3em; position:absolute; left:1%; cursor:pointer;"></i></div>';
            for(i = 1; i <= version ; i++){
                ver += `
                        <div class="vers">
                            <i class="fas fa-code-branch" style="margin-right: 7%; font-size: 2em; text-align:left; margin-left:3%;"></i>
                            <br>
                            Version ${i}
                        </div>`;
            }
...
<!-- Modal flujo -->
    ...
    <div id="modalInfoDocumento" class="modal fade">
        <div class="modal-dialog modal-lg mg-b-100" role="document" style="width: -webkit-fill-available;">
            <div class="modal-content tx-size-sm">
                <div class="modal-header pd-x-20" style="background:#848F33 !important; color:#fff !important;">
