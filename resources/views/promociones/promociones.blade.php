@php
    ...
    $arr_sigj=array('41PIC','44PIC','52PIC','57PIC','60PIC','61PIC', '200PIC', '100PIC', '300PIC' );
    $arr_sigj_materia=array('SF','SC');
    ...
    if(in_array($request->session()->get('juzgado_subtipo'), $arr_sigj_materia)){
        $bandera_sigj=0;
    }
...
@section('contenido-principal')
                        ...
                        <div class="row">
                                ...
                                <div class="col-lg-4">
                                    <div class="form-group mg-b-10-force">
                                        <label class="form-control-label">Tipo de documento:</label>
                                            ...
                                            <select class="form-control select2" id="tipo_documento" name="tipo_documento" data-placeholder="Choose one">
                                            

                                                @if($bandeja=="iniciales")
                                                <option value="demandas_todas">TODOS</option>
                                                <option value="INIC">DEMANDAS</option>
                                                @else
                                                <option value="POS">PROMOCIONES</option>
                                                @endif
                            ...
                            @else
                                @if($request->session()->get('juzgado_tipo')=='sala')
                                    <input type="hidden" value="POS_SALA" name="tipo_documento" id="tipo_documento">
                                @else
                                    <input type="hidden" value="POS" name="tipo_documento" id="tipo_documento">
                                @endif
                            @endif
        ...
        <div class="table-wrapper">
            ...
            <table id="datatable1" class="table display responsive wrap" style="max-width: 850px;">
                <thead>
                <tr>
                    <th class="wd-120p " >Fecha de recepción</th>
                    <th class="wd-40p" >{{$request->lang['Toca']}}/Tipo</th>
                    <th class="wd-10p romperCadena" >Partes</th>
                    <th class="wd-25p" style="width: 30px;">Archivos</th>
                    ...
                    @foreach ($lista["response"] as $resolucion)
                       <tr class="data-row-id-{{$i}}">
                        ...
                        <td class="romperCadena">
                            ...
                            @if($resolucion['opc_promocion_tipo_recepcion']!="")
                                @if($resolucion['opc_promocion_tipo_recepcion']=="exhodigital")
                                    <br><strong>TIPO RECEPCIÓN</strong><br>
                                    EXHORTO DISPONIBLE EN EL SISTEMA DE EXHORTOS WEB
                                @else
                                    <br><strong>TIPO RECEPCIÓN</strong><br>
                                    {{strtoupper($resolucion['opc_promocion_tipo_recepcion'])}}
                                @endif
                            @endif
                        ...
                        <td class="romperCadena">
                            @if($resolucion['opc_promocion_estatus']==1)
                                @if($resolucion['id_juicio']=="" and ($resolucion['tipo_expediente']!=357 and $resolucion['tipo_expediente']!=358 and $resolucion['tipo_expediente']!=360) and $resolucion['opc_promocion_origen']=="OPC" and $bandera_sigj)
                                    @if($bandeja=="iniciales")
                                ...
                                @else
                                    @if($resolucion['id_juicio']=="" and $resolucion['promocion_tipo_tramite_divorcio']!=1 and $bandera_sigj)
                                        @if($bandeja=="iniciales")
                                            @if($resolucion['opc_promocion_origen']=="OPC") 
                                                El documento llegará de forma física mediante OPC, en caso de existir un digital usted podrá visualizar los documentos una vez que haya creado el expediente en SICOR y confirmado en el SIGJ. 
                                            @else
                                        ...
                                        @else
                                            @foreach($resolucion['adjuntos'] as $adjuntos)
                                                @isset($adjuntos['json_arr']->idDocument)
                                                    @if($adjuntos_int==0)
                                                        <a href="javascript:void(0);" onclick="openDocumentoGestor({{$adjuntos['json_arr']->idDocument}},{{$resolucion['id_promocion']}});" ><span title="Comentarios: @isset($adjuntos['json_arr']->observaciones) {{$adjuntos['json_arr']->observaciones}} @else No hay observaciones @endif">@if(utilidades::buscarCatalogoBandera($request->entorno["catalogos"]["divorcio_expres_opc"], $resolucion['tipo_expediente']))Solicitud @else @if($bandeja=="iniciales") Demanda @else Promoción  @endif @endif</span></a>
                                                    @else
                                                        <br><a href="javascript:void(0);" onclick="openDocumentoGestor({{$adjuntos['json_arr']->idDocument}},{{$resolucion['id_promocion']}});"><span title="Comentarios: @isset($adjuntos['json_arr']->observaciones) {{$adjuntos['json_arr']->observaciones}} @else No hay observaciones @endif">Adjunto {{$adjuntos_int}}</span></a>
                                                    @endif
                                ...
                                @else
                                    ...
                                    @else
                                        Confirmada
                                            ...
                                            @isset($resolucion['datos_juicio']['juicio'][0]['id_juicio'])
                                                <small><br>{{$resolucion['datos_juicio']['juicio'][0]['expediente']}}/{{$resolucion['datos_juicio']['juicio'][0]['anio']}}
                                                    @if($resolucion['datos_juicio']['juicio'][0]['bis']!="")
                                                        Bis. {{$resolucion['datos_juicio']['juicio'][0]['bis']}}
                                                    @endif
                                                    <br>{{$resolucion['datos_juicio']['juicio'][0]['tipo_expediente']}}
                                                </small>
                                            @endisset
...
<script>
    ...
    function openDocumentoGestor(id, promocion_id=""){

        @if($request->session()->get('juzgado_tipo')=='sala')
            guardarConfirmacion('1', promocion_id)
        @endif
    ...
    function confirmarPromocion(promocion_id, expediente, tipo_juicio_id, promocion_tipo_tramite_divorcio, fecha, origen){
        ...
        $.ajax({
            ...
            error: function(XMLHttpRequest, textStatus, errorThrown) { 
                ...
                $('#modaldemo3').find('.modal-header').html('');
                $('#modaldemo3').find('.modal-body').html('<br><br><h3><center>Favor de intentar nuevamente</center><br></h3>');
                    ...
                    if(data.status==100){
                        ...
                        for(var i=0; i<data.response.length; i++){
                            ...
                            @if($request->session()->get('juzgado_subtipo')=="JFO" or 1)
                                ...
                                @else 
                                    if(data.response[i].confirmado=='no confirmado'){
                                        ...
                                        if(typeof(data.response[i]['datos_juicio']['juicio'][0]) !== "undefined" ){

                                            acciones+='<small><br>'+data.response[i]['datos_juicio']['juicio'][0]['expediente']+'/'+data.response[i]['datos_juicio']['juicio'][0]['anio'];
                                            if(data.response[i]['datos_juicio']['juicio'][0]['bis']!=null){
                                                acciones+=' Bis. '+data.response[i]['datos_juicio']['juicio'][0]['bis'];
                                            }
                                            acciones+='<br>'+data.response[i]['datos_juicio']['juicio'][0]['tipo_expediente']+'</small>';
                                        }
                                    }
                                @endif
                            @endif
                            ...
                            if(data.response[i].opc_promocion_estatus==1){
                                if(data.response[i].id_juicio==null && data.response[i]['tipo_expediente']!=357 && data.response[i]['tipo_expediente']!=358 && data.response[i]['tipo_expediente']!=360 && {{$bandera_sigj}}){
                                    @if($bandeja=="iniciales")
                                    ...
                                    if(data.response[i].id_juicio==null && data.response[i]['promocion_tipo_tramite_divorcio']!=1 && {{$bandera_sigj}}){
                                        @if($bandeja=="iniciales")
                                            if(data.response[i].opc_promocion_origen=="OPC"){
                                                documentos='El documento llegará de forma física mediante OPC, en caso de existir un digital usted podrá visualizar los documentos una vez que haya creado el expediente en SICOR y confirmado en el SIGJ.';
                                            }
                                        ...
                                        if(data.response[i].adjuntos.length==0){
                                            ...
                                            for(var j=0; j<data.response[i].adjuntos.length; j++){
                                                if(typeof(data.response[i].adjuntos[j]['json_arr']) !== "undefined" && data.response[i].adjuntos[j]['json_arr'] !== null ){
                                                    if(bandera_titulo==0){
                                                        if((data.response[i]['tipo_expediente']==357 || data.response[i]['tipo_expediente']==358 || data.response[i]['tipo_expediente']==360)){
                                                            documentos+='<a href="javascript:void(0);" onclick="openDocumentoGestor('+data.response[i].adjuntos[j]['json_arr'].idDocument+','+data.response[i].id_promocion+');" > @if($bandeja=="iniciales") Solicitud @else Promoción  @endif</a>';
                                                        }
                                                        else{
                                                            if(typeof(data.response[i].adjuntos[j]['json_arr'].idDocument) !== "undefined"){
                                                                documentos+='<a href="javascript:void(0);" onclick="openDocumentoGestor('+data.response[i].adjuntos[j]['json_arr'].idDocument+','+data.response[i].id_promocion+');" > @if($bandeja=="iniciales") Demanda @else Promoción  @endif</a>';
                                                            }
                                                            else{
                                                                documentos+='<a href="javascript:void(0);" onclick="openDocumentoGestor('+data.response[i].adjuntos[j]['json_arr'].idGlobal+','+data.response[i].id_promocion+');" > @if($bandeja=="iniciales") Demanda @else Promoción  @endif</a>';
                                                            }
                                                        }
                                                        bandera_titulo=1;
                                                    } 
                                                    else{
                                                        documentos+='<br><a href="javascript:void(0);" onclick="openDocumentoGestor('+data.response[i].adjuntos[j]['json_arr'].idDocument+','+data.response[i].id_promocion+');" >Adjunto '+j+'</a>';
                                                    }
                                                }
                                            }
                                        }
                            ...
                            */
                            ...
                            if(data.response[i]['opc_promocion_tipo_recepcion']!="" && data.response[i]['opc_promocion_tipo_recepcion']!=null){
                                if(data.response[i]['opc_promocion_tipo_recepcion']=="exhodigital"){
                                    expediente_txt+='<br><strong>TIPO RECEPCIÓN</strong><br>EXHORTO DISPONIBLE EN EL SISTEMA DE EXHORTOS WEB';
                                }
                                else{
                                    expediente_txt+='<br><strong>TIPO RECEPCIÓN</strong><br>'+data.response[i]['opc_promocion_tipo_recepcion'].toUpperCase();
                                }
                            }
