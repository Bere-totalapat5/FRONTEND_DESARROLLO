...
@section('contenido-pageheader')
    <ol class="breadcrumb slim-breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Búsqueda de publicación</li>
    </ol>
    <h6 class="slim-pagetitle">Bandeja búsqueda de publicación</h6>
@endsection
...
@section('contenido-principal')   
    <div class="section-wrapper">
        <div class="row">
            <div class="col-lg-11">
                @php 
                    $fecha_arr=explode('-', $request->proxima_publicacion['response']);
                    $meses = array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
                    $fecha_desktop=$fecha_arr[2] . ' de ' .$meses[$fecha_arr[1]-1] .' de ' . $fecha_arr[0];
                @endphp
                <h3 class="card-profile-name">Búsqueda de publicaciones</h3>
            </div>
            <div class="col-lg-1">
                <!--        
                                    ...
                                    <tr>
                                        ...
                                        <td style="width:30%;">
                                            <div class="input-group" style="width:100%;">
                                                ...
                                                <input type="text" class="form-control fc-datepicker" placeholder="MM/DD/YYYY" name="fecha_desde" id="fecha_desde" readonly="readonly" value="{{$lista_fecha_publicacion['response_publicacion']}}">
                                            </div>
                                            </td>
                                        <td> &nbsp; &nbsp; &nbsp;</td>
                                    </tr>
    ...
    <div class="table-wrapper">
        ...
        <table id="datatable1" class="table display responsive wrap" style="width: 100%; max-width: 850px;">
            <thead>
            <tr>
                <th class="wd-15p">Fecha de <br>publicación</th>
                <th class="wd-15p">{{$request->lang['Toca']}}<br>/Estado</th>
                <th class="wd-10p">{{$request->lang['Ponencia']}}</th>
            ...
            @isset($lista['response'][0]["id_acuerdo"])
                ...
                <tr class="data-row-id-{{$i}}">
                    ...
                    <td>
                        ...
                        @php
                            $fecha_actual = strtotime(date($resolucion["fecha_publicacion"]));
                            $fecha_entrada = strtotime(date('Y-m-d'));    
                        ...
                        @if($fecha_actual > $fecha_entrada)
                            ...
                            @if($sesion['id_tipo_usuario']=='1' or $sesion['id_tipo_usuario']=='4' or $sesion['id_tipo_usuario']=='5' or $sesion['id_tipo_usuario']=='6')
                                ...
                                <!--
                                <br><a href="javascript:void(0);" onclick="eliminarResolucion({{$resolucion['id_acuerdo']}}, {{$i}});">Cancelar publicación</a>
                                -->
...
<script>
    ...
    function eliminarResolucion(id_acuerdo, index){

        if(confirm('¿Esta seguro de eliminar el acuerdo?')){
    ...
    $(document).ready(function() {
        ...
        dataTableGlobal = table = $('#datatable1').DataTable( {
            ...
            { "targets": [1],  "orderable": false, "visible": true, 'className': 'romperCadena' },
        ...
        */
        ...
        $('#fecha_desde').val('{{$lista_fecha_publicacion['response_publicacion']}}');
    ...
    function accionBuscar_ajax(){
        ...
        $.ajax({
            type:'POST',
            url:'/bandejas/buscarPublicacion/buscar/',
            data:{toca:toca, anio_toca:anio_toca, asunto_toca:asunto_toca, fecha_desde:fecha_desde, fecha_hasta:fecha_desde },
            success:function(data){
                console.log(data);
                ...
                if(data.status==100){
                    for(var i=0; i<data.response.length; i++){
                        ...
                        fecha_arr2=fecha_arr[0].split("-");
                        var f1 = new Date(fecha_arr2[0], fecha_arr2[1]-1, fecha_arr2[2]); 
                        var f2 = new Date();
                        ...
                        if(f1.getTime() > f2.getTime()){
                            ...
                            @if($sesion['id_tipo_usuario']=='1' or $sesion['id_tipo_usuario']=='4' or $sesion['id_tipo_usuario']=='5' or $sesion['id_tipo_usuario']=='6')  
                                ...
                                if(comprarFechasMomento(fecha1, fecha2)){
                                    ...
                                    //acciones1 = '<br><a href="javascript:void(0);" onclick="eliminarResolucion('+data.response[i].id_acuerdo+', '+i+');">Cancelar publicación</a>';
                                }
    ...
    function imprimirTodo(){
        ...
        $.ajax({
            ...
            data:{ bandeja:'busquedaPublicacion', toca:$('#toca').val(), anio_toca:$('#anio_toca').val(), asunto_toca:$('#asunto_toca').val(), fecha_desde:$('#fecha_desde').val(), fecha_hasta:$('#fecha_desde').val()  },
            success:function(data){
                setTimeout(function(){
                    $('#modal_loading').modal('hide');
                }, 500);
