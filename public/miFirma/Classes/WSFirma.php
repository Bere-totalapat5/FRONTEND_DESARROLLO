    ...
    public function obtenerPDF($pdfBase64,$strSoapHeader="AuthSoapHd"){
        ...
        if($response->Error==0){
         ...
         if($pdfResult->status==0){
            ...
            $arr = array ('state'=>$pdfResult->status,'description'=>$pdfResult->description,"transfer"=>$iID,"date"=>$strFecha,"commonName"=>$strCommonName,"hexSerie"=>$strSerialNumber, "POST"=>$_POST);
            echo json_encode($arr); 
