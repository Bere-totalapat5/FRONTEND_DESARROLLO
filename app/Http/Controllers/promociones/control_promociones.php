...
class control_promociones extends Controller
{
    ...
    public function inicio( Request $request, $bandeja ){
        ...
        if($request->session()->get('juzgado_tipo')=='sala'){ 
            if($bandeja=='iniciales'){
                $datos['tipo_documento']='demandas_todas';
            }
        ...
        if(!isset($lista['response_pag']['pagina_actual'])){
    ...
    public function inicio_caratulas( Request $request ){

        unset($datos);
        $datos['juzgado']=$request->session()->get('usuario_juzgado');
        $datos['fecha_menor']='';
        $datos['fecha_mayor']='';
        $datos['fecha_simple']='';
        ...
        //se madna a la vista
        return view(    "promociones.caratula_promociones",
            ["entorno"=>$request->entorno,
            "request"=>$request,
            "sesion"=>$request->session()->all(),
            "menu_general"=>$request->menu_general,
    ...
    public function buscar_promocion_ajax( Request $request ){
        ...
        $datos['origen']=$input['medio_recepcion'];
    ...
    public function buscar_expediente_ajax( Request $request ){
        ...
        $bandera=1;
        $arr_sigj=array('41PIC','44PIC','52PIC','57PIC','60PIC','61PIC', '200PIC', '100PIC', '300PIC', '100JFO', '200JFO' );
        //if($request->session()->get('usuario_juzgado')=="100PIC" or $request->session()->get('usuario_juzgado')=="200PIC" or $request->session()->get('usuario_juzgado')=="300PIC" or $request->session()->get('usuario_juzgado')=="44PIC"){
        if(in_array($request->session()->get('usuario_juzgado'), $arr_sigj)){ 
            $bandera=0;
        }

        if($request->session()->get('juzgado_tipo')=='sala'){
            
            $lista_expediente=archivos::obtener_listado_sicor_salas($request, $datos);
            $plantilla_archivo_body=print_r($lista_expediente, true).'<br><br>'.print_r($datos, true).'<br><br>';
            $plantilla_archivo_header='';

            //return response()->json(['plantilla_archivo_header'=>$plantilla_archivo_header, 'plantilla_archivo_body'=>'SICOR_salas'.$plantilla_archivo_body]);

            include "plantilla_asignar_promocion_confirmar_sicor_salas.php";
            return response()->json(['plantilla_archivo_header'=>$plantilla_archivo_header, 'plantilla_archivo_body'=>'SICOR_salas'.$plantilla_archivo_body]);
        }



        if((($tipo_juicio_id!=357 and $tipo_juicio_id!=358 and $tipo_juicio_id!=360) or $origen=='OPC') and $bandera){
            $lista_expediente=archivos::obtener_listado_sicor($request, $datos);
            $plantilla_archivo_body=print_r($lista_expediente, true).'<br><br>'.print_r($datos, true).'<br><br>';
            $plantilla_archivo_header='';
            include "plantilla_asignar_promocion_confirmar_sicor.php";
            return response()->json(['plantilla_archivo_header'=>$plantilla_archivo_header, 'plantilla_archivo_body'=>'SICOR1'.$plantilla_archivo_body]);

        }
        else if($promocion_tipo_tramite_divorcio==0  and $bandera){
            $lista_expediente=archivos::obtener_listado_sicor($request, $datos);
            include "plantilla_asignar_promocion_confirmar_sicor.php";
            return response()->json(['plantilla_archivo_header'=>$plantilla_archivo_header, 'plantilla_archivo_body'=>'SICOR1'.$plantilla_archivo_body]);
        }
        else if(($tipo_juicio_id==357 or $tipo_juicio_id==358 or $tipo_juicio_id==360) and $promocion_tipo_tramite_divorcio==0 and $fecha<"2020-07-13" and $bandera){
            $lista_expediente=archivos::obtener_listado_sicor($request, $datos);
            include "plantilla_asignar_promocion_confirmar_sicor.php";
            return response()->json(['plantilla_archivo_header'=>$plantilla_archivo_header, 'plantilla_archivo_body'=>'SICOR1'.$plantilla_archivo_body]);
        }
    ...
    public function promocion_guardar( Request $request ){
        ...
        unset($datos);
        if($input['bandera_metodo_recepcion']==1){
        ...
        $datos['comentario']=!is_null($request['comentario']) ? $request['comentario'] : '' ;
        ...
        if(isset($lista['response']['id_promocion'])){
            ...
            //se hace la caratula
            if($input['bandera_metodo_recepcion']==2){
                ...
                if(isset($lista_expediente['response'][0]['datos_archivo']['id_juicio'])){
                    ...
                    $juicio='';
                    if(isset($lista_expediente['response'][0]['tipo_juicio'][0]['juicio'])){
                        $juicio=$lista_expediente['response'][0]['tipo_juicio'][0]['juicio'];
                    }
                    $id_catalogo_juicios=0;
                    if(isset($lista_expediente['response'][0]['tipo_juicio'][0]['id_catalogo_juicios'])){
                    ...
                    $metadatos=$lista['response']['id_promocion']."|6|1|1.0|$materia_opc|$materia_opc$numero|$expediente_anio|$expediente|$juicio|".$request->session()->get('juzgado_nombre_largo')."|".date('Y-m-d H:i:s')."|$actores|$demandado|$id_catalogo_juicios|".$lista['response']['id_promocion']."|sigj_promocion";
                    $metadatos=$lista['response']['id_promocion']."|6|1|1.0|$materia_opc|$materia_opc$numero|$expediente_anio|$expediente|$juicio|".$request->session()->get('juzgado_nombre_largo')."|".date('Y-m-d H:i:s')."|||$id_catalogo_juicios|".$lista['response']['id_promocion']."|sigj_promocion";
                    ...
                    $datos_promocion['opc_promocion_nombreCapturista']=$juicio;
                    $lista_editar=promociones::editar_promocion_metadatos($request, $lista['response']['id_promocion'], $datos_promocion);
                    //dd($lista_editar);



                    \QrCode::format('png')->size(800)->color(0,0,0)->backgroundColor(255,255,255)->encoding('UTF-8')->errorCorrection('L')->generate($metadatos, storage_path('app/temporales/'.$lista['response']['id_promocion'].'.png'));

                    include "caratula_qr/pdfCaratulaSIGJ.php";

                    //$qrGenerado = $oQR -> generaImagenQR($input['id_promocion'],$input['metadatos']);
                    $resultado = guardarPDF($lista['response']['id_promocion'], $metadatos, $datos['comentario'], $datos['fechaRecepcion']);
            ...
            else if($request->session()->get('juzgado_tipo')=='sala'){
                ...
                if(!utilidades::pingServer($url_simple, $url_simple_port)){
                    $request->session()->flash('error', '02 ERROR - Servicio apagado del gestor documental');
                    return back();
                }
    ...
    public function cargar_caratulas_pdf( Request $request ){
        ...
        if(isset($input['metadatos']) and isset($input['id_promocion'])){
            ...
            $lista=promociones::consultarCaratulasPendientes($request, $datos);
            

            \QrCode::format('png')->size(800)->color(0,0,0)->backgroundColor(255,255,255)->encoding('UTF-8')->errorCorrection('L')->generate($input['metadatos'], storage_path('app/temporales/'.$input['id_promocion'].'.png'));

            include "caratula_qr/pdfCaratulaSIGJ.php";

            //$qrGenerado = $oQR -> generaImagenQR($input['id_promocion'],$input['metadatos']);
            $resultado = guardarPDF($input['id_promocion'], $input['metadatos'], $lista['response'][0]['opc_promocion_comentario'], $lista['response'][0]['fecha_recepcion']);
    ...
    public function cargar_caratulas_pendinetes( Request $request ){
        $input = $request->all();
        ...
        $texto='<label class="section-title">No hay carátulas pendientes</label>';
        if(isset($lista['response'][0]['opc_promocion_id'])){
            $texto='<label class="section-title">Carátulas pendientes</label><ul class="list-group">';
            for($i=0; $i<count($lista['response']); $i++){
                $texto.='<li class="list-group-item">
                            <p class="mg-b-0"><strong class="tx-inverse tx-medium"><a href="javascript:void(0);" onclick="openDocumentoCaratula(\''.$lista['response'][$i]['opc_promocion_id'].'\', \''.$lista['response'][$i]['opc_promocion_metadata'].'\');">Folio: '.$lista['response'][$i]['opc_promocion_id'].'</a></strong> <span class="text-muted">Fecha: '.$lista['response'][$i]['fecha_registro'].'</span><br><span class="text-muted">'.$lista['response'][$i]['opc_promocion_comentario'].'</span> </p>
                        </li>';
            }
            $texto.='</ul>';
        }
    ...
    public function eliminar_caratula( Request $request ){
        $input = $request->all();

        unset($datos);
        $lista=promociones::eliminacion_promocion($request, $input['id_promocion']);

        return $lista;

    }

    public function editar_promocion_adjunto_visor( Request $request ){
        $input = $request->all();

        unset($datos);
        $lista=promociones::editar_promocion_adjunto_visor($request, $input['id_promocion'], $input['id_adjunto'], $input['status']);
    ...
    */
    public function gestor_guardar_doc( Request $request ){
        ...
        if(isset($input['id_expediente_opc'])){
            ...
            if(isset($input['sigj_promocion'])){
                ...
                $datos['bandera_buscar']=0;
                $bandera_guardar=1;
            }
            else if(isset($input['dato_extra'])){
                if($input['dato_extra']=='sigj_promocion'){
                    $datos['adjuntos'][]=[
                        "id_expediente_opc" => isset($input['id_expediente_opc']) ? $input['id_expediente_opc'] : 0,
                        "nombre" => "GESTOR SIGJ",
                        "hojas"=>$hojas,
                    ...
                    $arr_variables=explode('_', $input['dato_extra']);
                    //se consulta el flujo

                    $id_acuerdo=0;
                    if(isset($arr_variables[7])){
                        $id_acuerdo=$arr_variables[7];
                    }
                    $id_juicio=$arr_variables[6];
                    $id_organo=$arr_variables[2];

                    $arr_expediente="";
                    if(isset($arr_variables[8])){
                        $arr_expediente=explode('-', $arr_variables[8]);
                    }

                    $proceso = rand(100, 999);

                    if($arr_variables[0]=='acuerdo'){

                        //se guarda en el attachment
                        unset($datos);
                        $datos['adjuntos'][]=[
                            "id_expediente_opc" => isset($input['id_expediente_opc']) ? $input['id_expediente_opc'] : 0,
                            "nombre" => "ACUERDO EMP",
                        ...
                        $datos['bandera_buscar']=0;
                        $lista_attach=promociones::alta_adjunto_promocion($request, $input['id_documento'], $datos);

                        return $lista_attach;

                        //se consulta el flujo
                        $flujo=acuerdos::consulta_flujo_detalles_estadistica($request, $id_acuerdo, $id_organo);
                        if(isset($flujo['response']['creador'][0]['id_flujo_sala'])){

                            $id_documento=$flujo['response']['ultima_version_id'];
                            $id_usuario=$flujo['response']['creador'][0]['id_usuario'];
                            $id_flujo_sala=$flujo['response']['creador'][0]['id_flujo_sala'];


                            //se descarga el documento del sigj para luego subirlo
                            //$lista_descarga=bandejas::documento_descargar_sicor($request, $id_acuerdo, $organo_acuerdo, $id_documento, 'html');

                            //se descarga del gestor
                            $gestor64=gestorDocumental::getDocGestor($request, $input['idGlobal']);
                            $url_gestor='/var/www/html/sicor_extendido_80/public/temporales/test_'.$proceso.'.pdf';
                            if($gestor64['response']==100){
                                $pdf = fopen ($url_gestor,'w');
                                ...
                                $texto=[];
                                $texto["response"]=0;
                                $texto["mensaje"]="sin conexión con el gestor";
                                return response()->json($texto);
                            }

                            $wordBase64 = '-----------------------------------------------------------------------------------------------------';
                            $pdfBase64 = chunk_split(base64_encode(file_get_contents($url_gestor)));
                            $extension_word = 'docx';
                            
                            //se guarda al sigj
                            $lista_subir=bandejas::subir_documentos_flujo_sinse($request, $id_acuerdo, $id_organo, $id_flujo_sala, $wordBase64, $extension_word, $pdfBase64, $id_usuario);


                            return [$lista_subir, $lista_attach];
                        }
                        else{
                            $texto=[];
                            $texto["response"]=0;
                            $texto["mensaje"]="sin flujo";
                            return response()->json($texto);
                        }

                    }
                    else if($arr_variables[0]=='promocion'){

                        //se guarda en el attachment
                        unset($datos);
                        $datos['adjuntos'][]=[
                            "id_expediente_opc" => isset($input['id_expediente_opc']) ? $input['id_expediente_opc'] : 0,
                            "nombre" => "PROMOCION EMP",
                            "hojas"=>$hojas,
                            "data_json" => $data_json
                        ];
                        $datos['bandera_buscar']=0;
                        ...
                        //se consulta el flujo
                        $id_acuerdo=$arr_variables[7];
                        $organo_acuerdo=$arr_variables[2];
                        $fecha_recepcion=date("Y-m-d",strtotime($arr_variables[9]."- 1 days")); 

                        //se guarda nuevamente como si fuera upload
                        unset($datos);
                        $datos['origen']='SIGJ SCANER';
                        ...
                        $datos['juzgado']= '-' ;
                        $datos['idMateria']= '-';
                        $datos['noExpediente']=$arr_variables[3];
                        $datos['anioExpediente']=$arr_variables[4];
                        $datos['fechaRecepcion']= $fecha_recepcion ;
                        $datos['tipoJuicio']= '0' ;
                        $datos['reg_date']=$fecha_recepcion;
                        $datos['idExpediente']=$id_juicio;
                        $datos['id_juicio']=  $id_juicio;
                        $datos['tipoDocumento']= 'POS' ;
                        $datos['idGestor']= '0' ;
                        $datos['partes']['actores']=[];
                        $datos['partes']['demandados']=[];
                        $datos['adjuntos']=[];
                        ...
                        $datos['comentario']= '' ;
                        $lista_visible=promociones::guardar_promocion($request, $datos);


                        unset($datos);
                        $datos['adjuntos'][]=[
                            "id_expediente_opc" => isset($lista_visible['response']['id_promocion']) ? $lista_visible['response']['id_promocion'] : 0,
                            "nombre" => "PROMOCION EMP VISIBLE",
                            "data_json" => $data_json
                        ];
                        $datos['bandera_buscar']=0;
                        $lista_attach_visible=promociones::alta_adjunto_promocion($request, $lista_visible['response']['id_promocion'], $datos);
                        
                        //SE RELACIONA
                        unset($datos);
                        $datos['juzgado']=$id_organo;
                        $datos['id_acuerdo']=$id_acuerdo;
                        $datos['id_opc_promocion']=$lista_visible['response']['id_promocion'];
                        $lista_relacion=promociones::corelacion_acuerdos_demanprom_sinse($request, $datos);

                        return [$lista_relacion, $lista_visible, $lista_attach_visible];
    ...
    public function opc_promocion_guardar( Request $request ){
        ...
        if($header_key==$llave_acceso or 1){
            ...
            $datos['opc_promocion_bandera_mensaje']=!is_null($request['opc_promocion_bandera_mensaje']) ? $request['opc_promocion_bandera_mensaje'] : '0' ;
            $datos['id_juicio']=0;
            $datos["tipo_recepcion"] = ( isset($request["tipo_recepcion"]) and !is_null($request["tipo_recepcion"]) ) ? $request["tipo_recepcion"] : "-";
            $datos['comentario']= '' ;
            ...
            if($lista['status']==100){
                ...
                if(isset($lista['response'])){
                    $texto['error_response']=$lista['response'];
                }
            }
            return response()->json($texto);
        }
    ...
    public function estatus_escaneo_total( Request $request, $juzgado, $expediente, $anio ){

        unset($datos);
        $datos['juzgado']=$juzgado;
        $datos['fecha_menor']='';
        $datos['fecha_mayor']='';
        $datos['fecha_simple']='';
        $datos['sincronizados']='si';
    ...
    public static function getJuzgadoSicor($juzgado, $materia){
        ...
        $materias_arr = array("SF", "SC", "C", "F", "V", "U", "R", "B", "H");
