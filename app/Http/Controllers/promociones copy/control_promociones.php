...
class control_promociones extends Controller
{
    ...
    public function buscar_expediente_ajax( Request $request ){
        ...
        if((($tipo_juicio_id!=357 and $tipo_juicio_id!=358 and $tipo_juicio_id!=360) or $origen=='OPC') and 0){
        ...
        else if($promocion_tipo_tramite_divorcio==0 and 0){
            include "plantilla_asignar_promocion_confirmar_sicor.php";
            return response()->json(['plantilla_archivo_header'=>$plantilla_archivo_header, 'plantilla_archivo_body'=>'SICOR2'.$plantilla_archivo_body]);
        }
        else if(($tipo_juicio_id==357 or $tipo_juicio_id==358 or $tipo_juicio_id==360) and $promocion_tipo_tramite_divorcio==0 and $fecha<"2020-07-13" and 0){


            include "plantilla_asignar_promocion_confirmar_sicor.php";
            return response()->json(['plantilla_archivo_header'=>$plantilla_archivo_header, 'plantilla_archivo_body'=>'SICOR3'.$plantilla_archivo_body]);
        }
        ...
        if(isset($lista['response']['id_promocion'])){
            ...
            if($input['bandera_metodo_recepcion']==2){
                ...
                if(isset($lista_expediente['response'][0]['datos_archivo']['id_juicio'])){
                    ...
                    $resultado = guardarPDF($lista['response']['id_promocion'], $metadatos);
    ...
    public function cargar_caratulas_pdf( Request $request ){
        ...
        //$qrGenerado = $oQR -> generaImagenQR($input['id_promocion'],$input['metadatos']);
        $resultado = guardarPDF($input['id_promocion'], $input['metadatos']);
        ...
        if(isset($lista['response'][0]['opc_promocion_id'])){
            ...
            for($i=0; $i<count($lista['response']); $i++){
                $texto.='<li class="list-group-item">
                            <p class="mg-b-0"><strong class="tx-inverse tx-medium"><a href="javascript:void(0);" onclick="openDocumentoCaratula(\''.$lista['response'][$i]['opc_promocion_id'].'\', \''.$lista['response'][$i]['opc_promocion_metadata'].'\');">Folio: '.$lista['response'][$i]['opc_promocion_id'].'</a></strong> <span class="text-muted">Fecha: '.$lista['response'][$i]['fecha_registro'].'</span> </p>
                        </li>';
            }
            $texto.='</ul>';
        }
