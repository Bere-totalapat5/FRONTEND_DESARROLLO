    ...
    if(!isset($lista_expediente['response'][0]['datos_archivo']['id_juicio'])){
      ...
      if( ($expediente % 2) == 1 and $request->session()->get('grupo_trabajo_identificar_area')=='B'){
        ...
        <div class="table-responsive">
            <table class="table mg-b-0">
              ...
              for($i=0; $i<count($lista_expediente['response']); $i++){
                ...
                <tr>
                  ...
                  <td><a href="javascript:void(0);" onclick="seleccionarToca('.$lista_expediente['response'][$i]['datos_archivo']['id_juicio'].', \''.$expediente.'\', \''.$lista_expediente['response'][$i]['datos_archivo']['tipo_expediente'].'\', \''.$lista_expediente['response'][$i]['datos_archivo']['secretaria'].'\');">Asignar promoción</a></td>
                </tr>';
              }
              $texto.='
              </tbody>
          ...
          <script> 
              function seleccionarToca(id, texto, tipo, secretaria, id_acuerdo){
                  
                $("#id_acuerdo_promocion").val(id_acuerdo);
                $("#ponencia_evento").val(secretaria);
                $("#id_juicio_promocion").val(id);
                $("#juicio_promocion").val(texto);
                $("#informacion_toca").html("<h5 class=\"tx-success\">El archivo seleccionado: " + texto + " (" + tipo + ")</h5><br><br>" );
