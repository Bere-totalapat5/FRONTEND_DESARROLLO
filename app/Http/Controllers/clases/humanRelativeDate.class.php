			...
			protected function calcTimeDiffString(){
				...
				if($diff > 0){
					if($diff < $this->magic_5_mins){
						...
						$this->string = 'En los proximos minutos';
					} else if ($diff < $this->magic_30_mins){
						$this->string = 'En la proxima media hora';
					} else if ($diff < $this->magic_1_hour){
						$this->string = 'En la proxima hora';
					} else {
			...
			protected function calcDateDiffString(){
				...
				if($diff > 0){
					...
					if($diff >= $this->magic_1_day && $diff < ($this->magic_1_day * 2)){
						...
						if($event_day > $current_day){
							$this->string = '&Eacute;ste ' . $dias[date('N' , $this->event_timestamp_day)-1];
						} else {
							$this->string = 'Siguente ' . $dias[date('N' , $this->event_timestamp_day)-1];
						}
				...
				// Historical events
					...
					if($diff >= $this->magic_1_day && $diff < ($this->magic_1_day * 2)){
						...
						$this->string = 'El ' . $dias[date('N' , $this->event_timestamp_day)] . " pasado";
					} else if($diff <= ($this->magic_1_week * 2) ) {
						...
						if($month_diff == 0){
							...
							if($month_diff > 12){
								$this->string = 'Casi un a&ntilde;o';
							} else {
