...
#codigoeq = """IN ('100SF')"""
if len(sys.argv) > 3 and sys.argv[3] in ('1SC','2SC','3SC','4SC','5SC','6SC','7SC','8SC','9SC','10SC','1SF','2SF','3SF','4SF','5SF'):
...
def prepara_entrada(tipo,juicio,materia,pub,toca,otros,anotacion,tipo_expediente,cuaderno,especial,fecha_especial,pon,acdos):
    ...
    if tipo_expediente in ("amparo_directo","amparo_indirecto","amparo_exploratorio"):
        ...
        r += " Expdllo."
    elif tipo_expediente in ("amparo_incidente"):
        r += " INCIDENTE."
...
CONCAT(COALESCE(TRIM(p1.nombre),''), IF(p2.nombre <> "", CONCAT(' VS. ', COALESCE(TRIM(p2.nombre),'')), '')) AS actores,
...
if cursor.rowcount < 1:
    #print(qry);
    print( "Error: No hay publicaciones para: %s" % (fecha_qry,))
    sys.exit(1)
