...
def prepara_entrada(tipo,juicio,materia,pub,toca,otros,anotacion,tipo_expediente,cuaderno,especial,fecha_especial,pon,acdos):
    ...
    if otros:
        oya = []
        olist = [ x for x in sorted(otros.split(",")) if x != "toca" ]
...
GROUP BY z.orden, a.tipo, j.toca, j.anio_toca, j.asunto_toca, a.publicar_en, a.especial, j.tipo_expediente, TRIM( j.cuaderno )
