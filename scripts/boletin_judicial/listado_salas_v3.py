...
ptags = {
    ...
    "titacu_":"</titacuerdo>",
    "titsen":"<titsentencia>",
    "titsen_":"</titsentencia>"
}

if len(sys.argv) > 2 and sys.argv[2] == "lo":
    tags = ptags
...
with open(os.path.join(mytdir, listado),"wt", encoding='utf-8') as f:
    for tuple in cursor.fetchall():
        ...
        if jz != jz_:
            ...
            if jz_:
                f.write( tag("elsec",juzgado_autoridad[ jz_ ][1]) + "\n" +  tag("sec",juzgado_autoridad[ jz_ ][0]) + "\n")
            ov  = ""
            tp_ = ""
            jz_nvo = all_juzgados.popleft()
            while all_juzgados and jz_nvo != jz:
                f.write("\n" +  tag("sala",juzgado_nombres[ jz_nvo ]) + "\nNo hay publicaciones %s\n" % (fpub,))
                jz_nvo = all_juzgados.popleft()
            f.write("\n" +  tag("sala",juzgado_nombres[ jz ]) + "\n")
        if tipo != tp_:
            if ov: f.write(wout(ov))
            ov  = ""
            if tipo == 'acuerdo':
