...
codigoeq = """IN ('1SC','2SC','3SC','4SC','5SC','6SC','7SC','8SC','9SC','10SC')"""
if len(sys.argv) > 3 and sys.argv[3] in ('1SC','2SC','3SC','4SC','5SC','6SC','7SC','8SC','9SC','10SC','1SF','2SF','3SF','4SF','5SF','SC100','100SF'):
...
tipo_expediente = {
    ...
    "denuncia_incumplimiento_1_2019_v_toca": "CUADERNO FORMADO EN RELACIÓN AL ARTÍCULO 210 DE LA LEY DE AMPARO NÚMERO 1/2019",
    "denuncia_incumplimiento_1_2019_v_cuad_amparo": "CUADERNO FORMADO EN RELACIÓN AL ARTÍCULO 210 DE LA LEY DE AMPARO NÚMERO 1/2019"
}

same_toca   = re.compile('\d+-\d+-')
toca_valido = re.compile('^[\d-]+$')
num_punto   = re.compile(r'(?P<w>\d)\.,')

def prepara_entrada(tipo,juicio,materia,pub,toca,otros,anotacion,tipo_expediente,cuaderno,especial,fecha_especial,pon,acdos):
    # Abrev. juicio - materia
    if juicio in tjabrev:
        if toca == "592-2018-001":
            r = "Juicio Otros."
        else:
            r = tjabrev[ juicio ]
       #r = tjabrev[ juicio ]
    ...
    # Toca
    r += " T. " + toca
    if otros:     otros     = otros.replace('"',"").replace("'","").strip()
    ...
    if otros:
        oya = [toca,]
        olist = [ x for x in sorted(otros.split(",")) if x and x != toca and toca_valido.match(x) ]
        m = same_toca.match(toca)
        if m:
            pos = len(m.group(0))
        else:
            pos = 0
    ...
    if tipo_expediente in ("amparo_directo","amparo_indirecto","amparo_exploratorio"):
        ...
        if cuaderno:
            r+= " " + cuaderno + ","
    elif tipo_expediente in ("denuncia_incumplimiento_1_2019_v_cuad_amparo","denuncia_incumplimiento_1_2019_v_toca"):
        r += " CUADERNO FORMADO EN RELACIÓN AL ARTÍCULO 210 DE LA LEY DE AMPARO NÚMERO 1/2019"
    elif tipo_expediente in ("expedientillo","expedientillo_no_corresponde"):
        r += " Expdllo."
    if not (r.endswith('.') or r.endswith(",")):
        r += "."
    if countr:
        acdos = int(int(acdos)/countr)
        if acdos < 1: acdos = 1
        acdos = str(acdos)
    ...
    if especial:
        if especial == "no publicado":
            r += " No Publ."
        elif especial == "mal publicado" and not anotacion:
            r += " Mal Publ."
        if fecha_especial and not anotacion:
            try:
    ...
    m = num_punto.search(r)
    if m:
        repl = m.group('w') + ","
        r    = num_punto.sub(repl,r)
    if r.endswith(","):
        r = r[:-1]
    if not r.endswith("."):
        r += "."
...
COALESCE(TRIM(p3.nombre),'') AS terceria,
    ...
    GROUP_CONCAT(CONCAT_WS('-', j2.toca, j2.anio_toca, LPAD(j2.asunto_toca,3,'0')) SEPARATOR ',') AS otros,
	a.anotacion,
    j.tipo_expediente,
    IF( j.cuaderno IS NULL, "", TRIM(j.cuaderno) ) as cuaderno,
    a.especial,
    a.fecha_especial,
...
LEFT OUTER JOIN amparo_toca at      ON at.id_amparo =  a.id_juicio
LEFT OUTER JOIN juicio j2           ON at.id_toca   = j2.id_juicio
WHERE
      j.juzgado ''' + codigoeq + '''
  AND a.estatus = 30 -- ESTATUS_FIRMADO_Y_APROBADO
  AND a.activacion='%s'
-- GROUP BY j.toca, j.anio_toca, j.asunto_toca
GROUP BY z.orden, a.tipo, j.toca, j.anio_toca, j.asunto_toca, a.publicar_en, a.especial, j.tipo_expediente, IF( j.cuaderno IS NULL, "", TRIM(j.cuaderno) )
...
with open(os.path.join(mytdir, listado),"wt", encoding='utf-8') as f:
    ...
    for tuple in cursor.fetchall():
        ...
        (jz,tipo,tc,act,ter,juicio,materia,concepto,pub,toca,otros,anotacion,tipo_expediente,cuaderno,especial,fecha_especial,pon,acdos,vis) = t2
        # f.write("\n"+"\t".join(("--",jz,tipo,tc,act,ter,juicio,materia,concepto,pub,toca,otros,anotacion,tipo_expediente,cuaderno,especial,pon,acdos,vis,"\n")))
        if jz != jz_:
            # -- flush_??
            ...
            while all_juzgados and jz_nvo != jz:
                ...
                jz_nvo = all_juzgados.popleft()
            f.write(tag("sala",juzgado_nombres[ jz.upper() ]))
        if tipo != tp_:
            if ov: f.write(wout(ov))
            ov  = ""
            if tipo == 'acuerdo':
        ...
        if tc == tc_ or act == ac_:
            ov = ov.strip() + ", " + prepara_entrada(tipo,juicio,materia,pub,toca,otros,anotacion,tipo_expediente,cuaderno,especial,fecha_especial,pon,acdos)
        else:
            # -- flush_??
            if ov: f.write(wout(ov))
            if vis == 'secreto':
                ...
                if ter:
                    ov += " Tercero: " + ter
            ov  += " " + prepara_entrada(tipo,juicio,materia,pub,toca,otros,anotacion,tipo_expediente,cuaderno,especial,fecha_especial,pon,acdos)
