...
def fecha_boletin(fecha):
    ...
    try:
        ...
        if hoy in feriados or hoy.weekday() >4:
            print("Error: No hay publicación el ",str(hoy))
            sys.exit(1)
        f = inicio
        while f <= hoy:
        ...
        en_letras = num2letras(fqry.day) + " de " + mon[ fqry.month ].lower() + " del " + "dos mil " + num2letras(fqry.year % 100)
        fqry  = str(fqry.day) + " de " + mon[ fqry.month ] + " del " + str(fqry.year) 
    except Exception as e:
        print("Error en fecha (" + fecha + ") " + str(e))
        sys.exit(1)
    return(fpub,fqry,fecha,en_letras,num_boletin)

# -- # USAGE: fpub,fqry,fecha = fecha_boletin(op.fecha)
# -- 
