...
PLANTILLA="plantilla_boletin_20150913_conedictos.fodt"
...
if log_enabled: #debug(), info(), warning(), error() and critical()
    logger = logging.getLogger("coserBoletin")
...
def tcursopos(txt, search_text):
    ...
    try:
        if xFound is not None:
            pos =  xFound.getEnd()
            if 0:
                print("Found %s at %s" % (search_text, pos))
            try:
            ...
            print("No está %s en la plantilla" % search_text)
            sys.exit(1)
    except Exception as e:
        print("q6", str(e))
        sys.exit(1)
            ...
            try:
                ...
                for f in fi.readlines():
                    ...
                    if f.startswith("<"):
                        ...
                        elif f.startswith("<ect_ed_jzg>") or f.startswith("<Ect_ed_jzg>"):
                            br = True
                            print("<ect_ed_jzg>::",f)
                            cursor.ParaStyleName = "ect_ed_jzg"
                        elif f.startswith("<ect_ed_txt>") or f.startswith("<Ect_ed_txt>"):
                            br = True
                            print("<ect_ed_txt>::",f)
                            cursor.ParaStyleName = "ect_ed_txt"
                        elif f.startswith("<ect_ed_fin>") or f.startswith("<Ect_ed_fin>"):
                            br = True
                            print("<ect_ed_fin>::",f)
                            cursor.ParaStyleName = "ect_ed_fin"
                        elif f.startswith("<Edictos>") or f.startswith("<EDICTOS>"):
                            tc = tcursopos(newdoc,"ECT_Edictos")
                        else:
                            # desecha </.. <HTML </BODY
                            continue
                        if tc: # skip to next line
                            cursor.gotoRange(tc,False) # se va a sobreescribir la etiqueta
                            continue
