...
def prepara_entrada(tipo,juicio,materia,pub,toca,alias,especial,fecha_especial,pon,acdos):
...
GROUP BY z.orden, a.tipo, j.toca, j.anio_toca, j.asunto_toca, a.publicar_en, a.especial
...
with open(os.path.join(mytdir, listado),"wt", encoding='utf-8') as f:
    ...
    for tuple in cursor.fetchall():
        ...
        (jz,tipo,tc,act,ter,juicio,materia,concepto,pub,toca,alias,especial,fecha_especial,pon,acdos,vis) = t2
        # f.write("\n"+"\t".join(("--",jz,tipo,tc,act,ter,juicio,materia,concepto,pub,toca,alias,especial,pon,acdos,vis,"\n")))
        if jz != jz_:
            # -- flush_??
        ...
        if tc == tc_ or act == ac_:
            ov  += ", " + prepara_entrada(tipo,juicio,materia,pub,toca,alias,especial,fecha_especial,pon,acdos)
        else:
            # -- flush_??
            if ov: f.write(wout(ov))
            if vis == 'secreto':
