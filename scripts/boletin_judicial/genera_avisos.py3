...
import logging, getopt, uno, sys, os, re, base64, subprocess
...
qry = """SELECT CONCAT('/files/avisos_firmados/',ruta,TRIM(tipo_archivo))
FROM `aviso`
WHERE estatus IN ('por publicar','en publicacion')
  AND id_aviso IN (
         SELECT distinct id_aviso
           FROM aviso_publicacion
          WHERE fecha = DATE('%s')
         )
ORDER BY seccion,id_aviso ASC"""
...
def usage():
    sys.stderr.write("uso: python3 genera_avisos.py\n"+
                "       [-c  | --connection-string=]\n"+
                "       [-o  | --outfile=] \n"+
                "       file1 file2 ...\n")
...
def junta_odt(files,pdfOfile):
    ...
    if oproc.wait() != 0 or stderrdata:
        ...
        os.remove(tname)
    del oproc
    return "OK " + pdfOfile.replace('/var/www/public_html','')


cUrl = "file:///var/www/scripts/boletin_judicial/plantilla_boletin_20150912_avisos.fodt"
...
def main():
    ...
    if cuantos == 1:
        for x in done_impresos_order:
            r = done_impresos[x].replace('/var/www/public_html','')
            if log_enabled: logger.info("print %s" % r)
            print( r )
    elif cuantos > 1:
        mytdir = mkdtemp(prefix='rpt_',dir="/var/www/public_html/temporales")
        Avisos = os.path.join(mytdir,"Avisos.odt")
        errores = ""
        thisodt = []
        for x in done_impresos_order:
            if done_impresos[x].startswith("OK "):
                thisodt.append( done_impresos[x][3:] )
        if log_enabled: logger.info("merging: %s in %s" % ("+".join(thisodt), Avisos))
        junta = junta_odt( thisodt, Avisos )
        if log_enabled: logger.info("print %s" % junta)
        print(junta)
        if log_enabled: logger.debug("meged: %s" % Avisos)
    else:                      
        if log_enabled: logger.info("print Error: Ningún archivo")
        print("Error: Ningún archivo")
    if log_enabled: logger.debug("Session end.")
    sys.exit(0)
...
def imprime_uno(i):
    ...
    try:
        ...
        logger.error("Disconnecting:", e.Message)
        pass
    rv = "OK %s" % destFile.replace('file:///var/www/public_html','')
    # tiene firma ?
    if "/firmados/" in i:
        try:
            if log_enabled: logger.info("A firmar: %s" % i)
            try:
