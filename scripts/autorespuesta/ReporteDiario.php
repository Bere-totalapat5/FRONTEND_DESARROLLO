 ...
 */
    ...
    echo "Notificación de seguimientos " . date("Y-m-d H:i\n");
    ...
    require_once "$inc_dir/db/DBEnvioCorreo.php";
    ...
    $dbEnvioCorreo=   new DBEnvioCorreo();

    $fecha = date("Y-m-d");
    $fecha = TimeUtils::fechaLargaAMD($fecha);
    ...
    foreach ($usuarios as $usuario){
        ...
        $objEnvioCorreo = new ObjEnvioCorreo();
        $links = $usuario['descarga_mail']=='S';

        $objEnvioCorreo->id_usuario=$usuario['id_usuario'];
        $objEnvioCorreo->correo=$usuario['correo'];
        ...
        $mail->AltBody    = 
                ...
                $fecha, $expedientes);
        $objEnvioCorreo->mensaje = $mail->AltBody;
        $mail->MsgHTML(
                $correo->getHTML(
                $usuario['usuario'],
                $usuario['nombres'] . " " .$usuario['paterno'] . " " .$usuario['materno'],
        ...
        try{
            ...
            echo "\t" . $e->getMessage() . "(1)\n" ;
            $mail->ClearAddresses();
            $objEnvioCorreo->error=$e->getMessage();
            $dbEnvioCorreo->insertEnvioCorreo($objEnvioCorreo);
            continue;
        }
        ...
        if($fromCount==10){
            ...
            $objEnvioCorreo->error="Fallo de envio a la cuenta";
            $dbEnvioCorreo->insertEnvioCorreo($objEnvioCorreo);
