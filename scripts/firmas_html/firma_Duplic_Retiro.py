...
if sys.argv[2] == "Retira":
    print "OK"
    # logger.info("%s Retira" % sys.argv[1])
    try:
        data = os.stat(sys.argv[1])
        logger.debug("dest %s: %d %f" % (sys.argv[1],data.st_size, data.st_mtime))
    except:
        logger.error("Can't stat dest: %s" % sys.argv[1])
    sys.exit(0)
            ...
            for l in lineas:
                ...
                if k == "Firmante":
                    ...
                    if linea in all_html:
                        ...
                        lock1.release()
                        if os.path.isfile(sys.argv[2] +",lck"):
                            os.unlink(sys.argv[2] +",lck")
                        sys.exit(0)
