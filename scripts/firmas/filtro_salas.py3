...
if log_enabled: #debug(), info(), warning(), error() and critical()
    logger = logging.getLogger("filtro")
    hdlr = logging.FileHandler("/tmp/filtro.log")
...
def filtro_salida( DATA ):
    ...
    del DATA
    if soup is None:
        if log_enabled: logger.debug("No soup")
        return ""
    s = soup.find('body')
    if s is None:
        if log_enabled: logger.debug("No body in soup")
        return ""
    if 0:
    ...
    if debugis: print("\n" * 2)
    if debugis: print("=" * 66)
    if log_enabled: logger.debug("Done soup, %d chars" % (len(str(soup)),))
    return str(soup)
    # return str(soup)
    # --- ???? ----
...
def unpack_mht( file_name ):
    ...
    if 0:
        ...
        del data
        if log_enabled: logger.debug("Created .new")
    # with open( file_name + '.new' ) as fp:
    if 1:
        with open( file_name, "rt", encoding="latin1", errors="surrogateescape") as fp:
            try:
                if log_enabled: logger.debug("Extract MIME")
                m = email.message_from_file(fp)
            except Exception as e:
                if log_enabled: logger.error("Unpack: %s :: %s" % (file_name,str(e)))
                print("Error: en codificacion del MHT")
    if 0:
        os.unlink( file_name + '.new' )
        if log_enabled: logger.debug("Deleted .new")
    if not m:
        if log_enabled: logger.debug("Empty message")
        return ""
    if log_enabled: logger.debug("Got message")
    l = ""
...
def htm_from_LO( file_name ):
    ...
    try:
        ...
        if p == 0:
            ...
            if log_enabled: logger.error("Document conversion error.")
    except Exception as e:
        print("Error",str(e))
        if log_enabled: logger.error("Error: unoconv: %s" % str(e))
        sys.exit(1)
    if log_enabled: logger.debug("Done converting")
    return filtro_salida(l)

if __name__ == "__main__":
    params = ", ".join( sys.argv[1:] )
    if log_enabled: logger.debug("GOT params: %s" % params)
    l = ""
    is_mht = False
    infile = sys.argv[1]
    ...
    if infile.endswith(".mht"):
        ...
        with open(infile,'r', encoding="latin1", errors="surrogateescape") as f:
            ...
            if m:
                is_mht = True
                l = ""
                if log_enabled: logger.debug("is mht2")
            elif h:
                with open( infile + '.new', 'w', encoding="utf-8", errors="surrogateescape") as f:
                    f.write(l)
                    if log_enabled: logger.debug("wrote: %s" % infile + '.new')
                l = ""
                with open( infile + '.new' ) as f:
                    l = f.read()
                    if log_enabled: logger.debug("To FS")
                    l = filtro_salida(l)
                    if log_enabled: logger.debug("From FS")
                    # os.unlink( infile + '.new' )
                if log_enabled: logger.debug("deleted: %s" % infile + '.new')
            else:
                print("Unknown file type")
                l = ""
    if is_mht:
        if log_enabled: logger.debug("To unpack")
        l = unpack_mht( infile ) # This is .mht --> unpack and load main file
        if log_enabled: logger.debug("unpacked mht")
    if l:
        infile = sys.argv[2]
        ...
        try:
            with open(infile,"wb") as f:
                ...
                if log_enabled: logger.debug("File: %s OK" % infile)
                print("OK")
            if log_enabled: logger.info("OK")
            sys.exit(0)
        except Exception as e:
            print("Error(5) %s" % sys.exc_info()[0])
            if log_enabled: logger.error("Error(5) %s" % sys.exc_info()[0])
    else:
        print("Error no es posible extraer el contenido")
        if log_enabled: logger.error("Error no es posible extraer el contenido")

if log_enabled: logger.error("Exit 7")
sys.exit(7)
