...
def filtro_salida( file_name ):
    ...
    for find_this_style in ('p','span','table','img'):
...
re_head = re.compile(r"</head>\s*<body>")
        ...
        if isfile(i):
            ...
            for l in rLines:
                ...
                if missing_instance == 0 and instance:
                    m = re_head.search(l)
                    if m:
                        l = re_head.sub(r'</head><body>',l)
                        l = l.replace("</head><body>",
            ...
            if missing_instance == 0 and instance:
                ...
                if m:
                    allfile = re_head.sub(r'</head><body>',allfile)
                    allfile = allfile.replace("</head><body>",
                           "</head><body><h1>" + instance + "</h1>",1)
                    missing_instance = 1;
