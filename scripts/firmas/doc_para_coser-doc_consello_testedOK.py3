...
from sello_firma_al_margen import firma_pdf
...
def main():
    ...
    if cuantos == 1:
        ...
        print(junta_pdf( thispdf, oPDF ))
        if log_enabled: logger.debug("meged: %s" % oPDF)
    else:                      
        print("Error: Ningún archivo")
    if log_enabled: logger.debug("session end. OK")
    sys.exit(0)
