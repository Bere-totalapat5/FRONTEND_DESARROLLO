...
def filtro_salida( DATA ):
    ...
    [ s.extract() for s in soup('meta') if not "charset" in s['content']]
    for s in soup('o:p'):
        s.name = 'p'
    ...
    for s in soup(class_=re.compile(r'FootnoteText',re.I)):
        ...
        s.a.extract()
        if verbose: print("Text:",s.get_text())
        ...
        d['data-created'] = "26/Feb/2015 13:35"
        s.unwrap()
        p = d.parent
        p['class'] = 'notas'
        try:
...
def unpack_mht( file_name ):
    m = ""
    with open(file_name, 'r', encoding="windows-1252", errors="surrogateescape") as f:
        data = f.read()

    with open(file_name + '.new', 'w',
...
if __name__ == "__main__":
    ...
    if infile.endswith(".mht"):
        ...
        with open(infile,"r") as f:
            l = f.read()
            if re.search(r'MIME-Version', l, 0, 2048):
                is_mht = True
                l = ""
            elif re.search(r'<[a-z]+>', l, 0, 8192):
                l = filtro_salida(l)
            else:
