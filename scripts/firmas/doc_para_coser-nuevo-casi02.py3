...
def filtro_salida( file_name ):
    ...
    for find_this_style in ('p','span','table'):
        for s in soup(find_this_style):
            if s.has_attr('style'):
                sty = s['style']
                if debugis: print(find_this_style + "::\n---" + sty)
                #if sty.find("font-family") >= 0:
                sty = re.sub(r'mso-bidi-','',sty)
                sty = re.sub(r'font-weight: normal;\s?','',sty)
                sty = re.sub(r'font-style: normal;\s?','',sty)
                sty = re.sub(r'font-size[^;]*;\s?','',sty)
                sty = re.sub(r'line-height[^;]*;\s?','',sty)
                sty = re.sub(r'font-size[^;]*;\s?','',sty)
                sty = re.sub(r'margin-left: auto; margin-right: auto;\s?','align: center; ',sty)
                if "font-family" in sty:
                    if debugis: print("--- " + sty)
                    sty = re.sub(r'font-family[^;]*;\s?','',sty)
                #if debugis: print("    " + sty)
                s['style'] = sty
