...
import logging, getopt, uno, sys, os, re, base64, subprocess, shutil
...
def junta_pdf(files,pdfOfile):
    ...
    if oproc.wait() != 0 or stderrdata:
        ...
        os.remove(tname)
    del oproc
    f = pdfOfile.replace('/var/www/public_html','')
    ...
    for pathname in args:
        if isfile(pathname):
            ...
            if pathname.endswith(".doc"):
                ...
                expected_filename = ""
            found_in_cache = 0
            if expected_filename and isfile(expected_filename):
            ...
            print("Error: Problemas para localizar el archivo")
