...
try:
    if sys.argv[1] != sys.argv[2]:
        p = os.path.dirname(sys.argv[2])
        # existe la carpeta de destino ?
        if not os.path.exists(p): # crea el directorio si es necesario
            os.makedirs(p)
        if os.path.isdir(p):
        # agregar a la rutina del cache expirar pdfs firmados en porfirmar
