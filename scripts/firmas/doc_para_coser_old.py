...
def main():
    cUrl = "file:///var/www/scripts/firmas/tsjdf_plantilla20140107_toficio.ott"
    outputfile = "resoluciones.pdf"
    retVal = 0

    opts, args = getopt.getopt(sys.argv[1:], "hco:", ["help", "connection-string=", "outfile" ])
    url = "uno:socket,host=mydbserv,port=2002;urp;StarOffice.ComponentContext"
    ...
    try:
        ...
        if log_enabled: logger.error("UnoException: " + e.Message)
    except RuntimeException as e:
        if log_enabled: logger.error("RuntimeException: " + e.Message)
    except NoConnectException as e:
        if log_enabled: logger.error("NoConnectException: " + e.Message)
    except IllegalArgumentException as e:
        if log_enabled: logger.error("IllegalArgumentException: " + e.Message)
    except IOException as e:
        if log_enabled: logger.error("IOException: " + e.Message)
    except:
        if log_enabled: logger.error("except: Could not create a new doc")
        sys.exit(1)
    ...
    try:
        ...
        if debugis: newdoc.storeAsURL(destFodt,oProps) # (PropertyValue("FilterName",0, "ODF Text Document", 1),))
