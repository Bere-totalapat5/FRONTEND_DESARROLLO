...
this_config   = "sicor"
watermarkfile = "/san/www/scripts/firmas/plantilla_sellosQR_v7.fodt"
# [sicor2]
DB_host       = "172.16.159.141"
DB_user       = "sicor"
...
def sellos_firmas(pathname,pdfOfile):
    ...
    # obtener sha256 del archivo
    m = re.search(r'/([0-9]+)(vp)?\.',pathname)
    if m:
        id_acuerdo = m.group(1)
        vp = m.group(2)
    else:
        print("Error: Imposible extraer el num de acuerdo")
        return('','')
    # obtener nas
    nas = getnas(id_acuerdo)
    sha256 = check_hash(pathname)
    (m,ext) = os.path.splitext(pathname)
    if ext in (".htm",".html"):
        try:
            extrae_firmas(pathname)
        except Exception as e:
            print("Error: Falla lectura de firmas")
            return('','')
    else:
        # print(ext,m,pathname)
        # print("falló extracción de firmas")
        pass
    # TODO: ??? Advertir que sha256 coincida
    juzgado,expediente,acuerdo,fecha_pub,gestor_documental = get_acuerdo(id_acuerdo)
    # obtener firmas
    fecha_cert, firmas = get_firmas(id_acuerdo, sha256, vp)
    # crear imagen para Q.R.
    (dir,n) = os.path.split(pdfOfile)
    qrimg   = makeqr(dir,id_acuerdo,sha256,gestor_documental)
...
try:
    import MySQLdb
    db = MySQLdb.connect( host="172.16.159.141",
                           db=DB_name,
                           user=DB_user,
                           passwd=DB_pass,
                           charset="utf8",
    ...
    print("Error: Carga driver de la base de datos: %s" % str(e)) # Failed to load MySQLdb
    sys.exit(1)
