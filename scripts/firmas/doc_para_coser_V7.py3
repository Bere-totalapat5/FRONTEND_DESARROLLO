...
mytdir = mkdtemp(prefix='rpt_',dir="/san/www/html/front_penal_desarrollo/storage/temp_exportaciones_CJ")
...
def main():
    #print(sys.argv[2:])
    firmas_parciales = 0
    nombre_archivo = ' '.join(sys.argv[2:])
    #print(nombre_archivo)
    opts, args = getopt.getopt(sys.argv[1:], "hcof:", ["help", "connection-string=", "outfile","firmas-parciales","coser","name"])
    for o, a in opts:
        if o in ("-h", "--help"):
            usage()
            sys.exit()
        ...
        if o in ("-p", "--coser"):
            ...
            for pathname in args:
                if isfile(pathname):
                    thispdf.append( pathname )
            mytdir = mkdtemp(prefix='rpt_',dir="/san/www/html/front_penal_desarrollo/public/CJ/temporales")
            oPDF = mytdir + "/unidos.pdf"
    ...
    for pathname in args:
        if isfile(pathname):
            # check cache
            # expected_filename=re.sub("\.docx?$",".pdf",pathname)
            ...
            if firmas_parciales and "/porfirmar/" in expected_filename:
                expected_filename = expected_filename.replace(".pdf","2.pdf")
            found_in_cache = 0
            if expected_filename:

                path = "/san/www/html/front_penal_desarrollo/storage/app/firmados/firma.txt"
            ...
            if found_in_cache:
                ...
                if USE_WINDOWS and (pathname.endswith('doc') or pathname.endswith('docx')):
                    ...
                    #print("aqui 7")
                elif pathname.endswith('pdf'):
                    #print("pdf")
                    expected_filename=pathname
                    ...
                    if isfile(gotfile):
                        ...
                        if "/firmados/" in pathname or (firmas_parciales and "/porfirmar/" in pathname):
                            try:
                                if log_enabled: logger.info("A firmar arhivo: %s" % pathname)
                                destFile = expected_filename
                                try:
                                    destFile = firma_pdf(nombre_archivo, expected_filename)
                                    #print(nombre_archivo)
            ...
            if log_enabled: 
                if isfile(pathname.replace("firmados","porfirmar")):
                    logger.error("IOException: 3 (not a file) %s", pathname)
                    logger.error("se copiara: %s a %s", pathname.replace("firmados","porfirmar"), pathname)
                    shutil.copy(pathname.replace("firmados","porfirmar"), pathname)
    ...
    if cuantos == 1:
        for x in done_impresos_order:
            if x in done_impresos and "/files/" in done_impresos[x]:
                ...
                #logger.info("pasamos el else2 %s" % done_impresos[x])
...
def imprime_uno(i):
    ...
    rv = "OK %s" % f 
    # tiene firma ?
    if "/firmados/" in i or (firmas_parciales and "/porfirmar/" in i):
        try:
