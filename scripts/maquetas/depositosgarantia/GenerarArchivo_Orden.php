...
require_once("/var/www/depgar/php-inc/db/mysql/DBPersona.php");
...
while ( true ) {
  ...
  $idOrdenRequest = $_GET["idOrden"];
  //Obtener tranasacciones
  $data = $dbPersona->obtenerPersonasTransaccionPorPublicarUnaIdOrden($idOrdenRequest);
  ...
  foreach ($data as $row ){
    ...
    fwrite2($file, $row['transaccion']->fecha_limite);               //(5)Fecha límite de pago
    ...
    fwrite2($file, str_pad(str_replace('.', '', $row['transaccion']->importe),15,   //(8)Importe de la operacion
			   '0',STR_PAD_LEFT));
    fwrite2($file, str_pad($row['transaccion']->idPersona,16,'0',STR_PAD_LEFT)); //(9)Númerode orden
    fwrite2($file, "80");                                            //(10)tipo de operacion
    fwrite2($file,$row['transaccion']->fecha_aplicacion);            //(11)Fecha de aplicación
