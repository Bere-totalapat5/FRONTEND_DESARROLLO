 ...
 */
    ...
    echo "VerificaOrdenesPendientes: " . date("Y-m-d H:i") . "\n";
    define("SCRIPT",1);
    //define ("BASE_DIR","C:/Program Files/Apache Software Foundation/Apache2.2/SIBJDF");
    define ("BASE_DIR","");
   ...
   $dbPersona = new DBPersona();
    ...
    while( false !== ($archivo = readdir($dirGestor)) ) {
        ...
        while( ($line = fgets($fileGestor)) !==false ) {
            ...
            $numeroOrden = $dbPersona->existeIdPersona($data[1]);
            if( $numeroOrden!== false){
                $fecha = substr($data[10],6,4) . "-" . substr($data[10],3,2) . "-" .substr($data[10],0,2);


                $dbPersona->actualizarEstatus($numeroOrden, $data[12]);
                $dbPersona->actualizarUltimoCambio($numeroOrden, $fecha);
            }
